...
def mf_to_raw(
    x: FloatTensor | NDArray[np.float32],
    exponent_bit_width: int,
    mantissa_bit_width: int,
    exponent_bias: int,
    eps: float,
) -> IntTensor | NDArray[np.int_]:
...
class MinifloatQuantizer(nn.Module):
    def __init__(
        ...
        self.exponent_bias = cast(int, exponent_bias)
        self.eps = eps
        self.ident = qnn.QuantIdentity(
            act_quant=fp_mixin_factory(
                exponent_bit_width,
                mantissa_bit_width,
                FloatActBase,  # pyright: ignore[reportArgumentType]
            )
        )
